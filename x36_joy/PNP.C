        ...
        switch(IrpStack->MinorFunction)
        {
            case IRP_MN_START_DEVICE:
                ...
                if(NT_SUCCESS (ntStatus))
                {
					...
					RtlInitUnicodeString(&devname,L"\\Device\\X36_Kb");
					ntStatus = IoGetDeviceObjectPointer(&devname,GENERIC_READ,&DeviceExtension->xfo,&DeviceExtension->xdo);
                                        if(NT_SUCCESS(ntStatus)) { ntStatus = HGM_InitDevice (DeviceObject, Irp); } else { DeviceExtension->xfo = NULL; }
                }
